...
public class TranslationDatabase extends SQLiteOpenHelper {
    ...
    */
    ...
    public void onCreate(SQLiteDatabase db) {
        String createTable = "CREATE TABLE " + TABLE_NAME + " (" + COL1 + " TEXT, " + COL2 +
                             " INT, " + COL3 + " INT," + COL4 + " TEXT," + COL5 + " TEXT);" ;
        db.execSQL(createTable);
    }

    /*
    necessary onUpgrade function
    */
    @Override
    ...
    */
    public boolean checkChapter1Existence(String book, Integer translation){
        ...
        // make the query
        String Query = "SELECT * FROM " + TABLE_NAME + " WHERE " + COL1 + " = '" + book+ "' AND " +
                       COL2 + "=  1 AND " + variable_column + " IS NOT NULL;";
    ...
    */
    public void addItem( String book, int chapter, int verse, String text, int translation) {
        ...
        {
            query = "UPDATE " + TABLE_NAME + " SET " + variable_column + " = '" + text +
                    "' WHERE " + COL1 + " = '" + book+ "' AND " + COL2 + " = " + chapter +
                    " AND " + COL3 + " = " + verse +  ";";
        }
        else{
            query = "INSERT INTO " + TABLE_NAME + "(" + COL1 + ", " + COL2 + ", " + COL3 + ", " +
                    variable_column + ") VALUES( '" + book + "', " + chapter + ", " + verse +
                    ", '" + text + "');";
        }
    ...
    */
    public Cursor getMaxVerse(String book, int chapter){
        SQLiteDatabase db = this.getWritableDatabase();
        String query;
        query = "SELECT MAX (" + COL3 + ") FROM " + TABLE_NAME + " WHERE " + COL1 + " = '" + book +
                "' AND " + COL2 + "= " + chapter + " ;";
    ...
    */
    public Cursor getChapter(String book, Integer chapter, Integer translation) {
        ...
        SQLiteDatabase db = this.getWritableDatabase();
        String query = "SELECT * FROM " + TABLE_NAME + " WHERE " + COL1 + " = '" +  book + "' AND " +
                       COL2 + " = " + chapter + " AND " + variable_column + " IS NOT NULL;";
    ...
    */
    public Cursor getVerses(VerseClass verse_class) {
        SQLiteDatabase db = this.getWritableDatabase();
        String query = "SELECT * FROM " + TABLE_NAME + " WHERE " + COL1 + " = '" + verse_class.book +
                       "' AND " + COL3 + " BETWEEN " + verse_class.begin_verse + " AND " +
                       verse_class.end_verse + " AND " + COL2 + " = " + verse_class.chapter + ";";
        return db.rawQuery(query, null);
    }
}